...
// incluirla en el directorio de tu proyecto y mandarla llamar como: #include "display.h"    
...
/* en un LCD fisico (no es critico para simulcacion solo en Proteus*/
void ConfiguraLCD()
{
...
/* Procedimiento que manda un string variable (que cambie durante la ejecuci�n) */
...
/* Mueve el cursor a la poscicion (x,y)  */
/*   x: numero de columna 0 a 19         */
/*   y: numero de renglon 0,1,2 � 3      */
...
/* Procedimiento que CreaCaracter en la memoria RAM del LCD 
  Parametros:   NoCaracter.- numero entre 0 y 7 (puedes tener 8 caracteres diferentes 
  ...
  Ejemplo:  unsigned char Letra0[8] = {0x00, 0x0A, 0x15, 0x11, 0x0A, 0x04};   //Caracter como un corazon
            CreaCaracter(0, Letra0);
                        
  Para desplegar el caracter especial se pondria: LetraLCD(0); (el 0 cambiaria por el numero de letra si es el caso)
  
  Importante: cuando mandas llamar CreaCaracter el cursor se mueve a la primera poscici�n del LCD 
*/                       
